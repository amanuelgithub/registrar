...
export class TestsComponent implements OnInit , AfterViewInit{
  ...
  constructor(private courseService: TestService, private messageService: MessageService) {
    this.getTests();
   }
  ...
  getTests(): void{
    this.courseService.getTests().subscribe(tests => this.dataSource.data = tests);
  }

  add(num:number, time: string): void{
    time = time.trim();
    if(!time) {return;};
    this.courseService.addTest({num, time} as Test)
        .subscribe(course => {
